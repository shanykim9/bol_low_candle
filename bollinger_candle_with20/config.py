...
SCAN_MONTH_STEP = 6
SCAN_MONTH_MIN  = 6
SCAN_MONTH_MAX  = 120          # 상대기간 최대 10년
SCAN_RANGE_YEARS_MAX = 10      # 특정기간: 최대 10년 전 ~ 현재
SCAN_MODES = ["상대기간", "특정기간"]
TRADING_DAYS_PER_MONTH = 21   # 연간 약 252거래일 기준
MAX_CANDLE_FETCH = 3000        # 키움 API 일봉 최대 수집량 (10년+MA 여유)
...
DEFAULT_PARAMS = {
    ...
    "scan_mode"        : "상대기간",   # 상대기간 | 특정기간
    "scan_months"      : 6,           # 상대기간: 탐색 개월 (6개월 단위, 최대 120=10년)
    "scan_start"       : "",          # 특정기간: 시작 년월 "YYYY-MM"
    "scan_end"         : "",          # 특정기간: 종료 년월 "YYYY-MM"
...
def scan_month_options() -> list[int]:
    """6개월 단위 탐색 옵션 (6 ~ 120개월)"""
    return list(range(SCAN_MONTH_MIN, SCAN_MONTH_MAX + 1, SCAN_MONTH_STEP))


def scan_mode_options() -> list[str]:
    return list(SCAN_MODES)


def parse_year_month(ym: str) -> Optional[date]:
    """'YYYY-MM' → 해당 월 1일. 실패 시 None."""
    if not ym:
        return None
    try:
        return datetime.strptime(str(ym).strip()[:7], "%Y-%m").date().replace(day=1)
    except ValueError:
        return None
...
def scan_range_min_date(today: Optional[date] = None) -> date:
    """특정기간 선택 가능 최소일 (오늘 기준 최대 10년 전 월초)"""
    t = today or date.today()
    y, m = t.year - SCAN_RANGE_YEARS_MAX, t.month
    return date(y, m, 1)


def default_scan_start_ym(today: Optional[date] = None) -> str:
    """기본 시작 년월: 6개월 전"""
    t = today or date.today()
    y, m = t.year, t.month - 6
    while m <= 0:
        m += 12
        y -= 1
...
def default_scan_end_ym(today: Optional[date] = None) -> str:
    t = today or date.today()
    return f"{t.year:04d}-{t.month:02d}"


def resolve_scan_range(params: dict) -> Tuple[Optional[date], Optional[date]]:
    """
    특정기간 모드일 때 (시작일, 종료일) 반환.
    상대기간 모드이거나 미설정이면 (None, None).
    """
    if params.get("scan_mode") != "특정기간":
        return None, None
    start = parse_year_month(params.get("scan_start", ""))
    end_m = parse_year_month(params.get("scan_end", ""))
    if start is None or end_m is None:
        return None, None
    end = month_end(end_m)
    if start > end:
        return None, None
...
def resolve_scan_days(params: dict) -> int:
    """
    상대기간: scan_months → 거래일.
    특정기간: 시작~종료 달력일 기준 대략 거래일 수.
    """
    start, end = resolve_scan_range(params)
...
def calc_data_days_from_params(params: dict) -> int:
    """params 기준으로 일봉 수집 일수 계산"""
    ...
    if start is not None and end is not None:
        ...
        span = (fetch_end - start).days + 1
        needed = ma_period + int(span * 252 / 365) + 30
        return min(MAX_CANDLE_FETCH, max(MIN_CANDLE_FETCH, needed))
    return calc_data_days(int(params.get("scan_months", SCAN_MONTH_MIN)), ma_period)


def format_scan_period(params: dict) -> str:
    """UI/로그용 탐색 기간 설명"""
    start, end = resolve_scan_range(params)
    if start is not None and end is not None:
        return f"{start.strftime('%Y-%m')} ~ {end.strftime('%Y-%m')}"
    return format_scan_months(int(params.get("scan_months", SCAN_MONTH_MIN)))
...
def resolve_data_days(params: dict) -> int:
    ...
    auto     = calc_data_days_from_params(params)
    return max(auto, explicit) if explicit > 0 else auto


# 기본 data_days 자동 설정
DEFAULT_PARAMS["data_days"] = calc_data_days_from_params(DEFAULT_PARAMS)
DEFAULT_PARAMS["scan_start"] = default_scan_start_ym()
DEFAULT_PARAMS["scan_end"]   = default_scan_end_ym()
