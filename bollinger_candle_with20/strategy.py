...
from config import resolve_scan_days, resolve_scan_range
...
def _date_in_range(val, start, end) -> bool:
    """캔들 날짜가 [start, end] 구간에 있는지 (start/end는 date)"""
    ts = _to_date(val)
...
def _is_pivot_row(df: pd.DataFrame, idx: int, params: Optional[dict] = None) -> bool:
    ...
    저점캔들 행 조건:
      공통) 저가 >= MA
           AND (시/고/저/종 중 하나라도 BB하단 이하)  — 양봉·음봉 무관
...
def find_pivot_candle(df: pd.DataFrame, scan_days: int = 126,
    ...
    """
    저점캔들 탐색
    - 상대기간: 최근 scan_days 거래일 이내
    - 특정기간: scan_start ~ scan_end 구간 내
    조건:
      A) 저가 >= 240일 이동평균
      B) 시/고/저/종 중 하나라도 BB하단 이하 (양봉·음봉 무관)
      C) (240선추세 모드) MA 연속 상승 N일 이상
    """
    if df.empty or len(df) < 2:
        return None

    start, end = resolve_scan_range(params or {})
    if start is not None and end is not None:
        search_idxs = [
            idx for idx in df.index[:-1].tolist()
            if _date_in_range(df.loc[idx, "date"], start, end)
        ]
    else:
        search_idxs = df.iloc[-(scan_days + 1):-1].index.tolist()

    for idx in reversed(search_idxs):
...
def simulate_strategy(candles: list[dict], params: dict) -> list[dict]:
    ...
    scan_start, scan_end = resolve_scan_range(params)
    ma_period = params["ma_period"]
    bb_period = params["bb_period"]
    ...
    i = min_idx
    while i < len(df):
        bar_date = _to_date(df.loc[i, "date"])
        if bar_date is not None and not pd.isna(bar_date):
            d = bar_date.date()
            if scan_start is not None and d < scan_start:
                i += 1
                continue
            if scan_end is not None and d > scan_end:
                break
...
def _find_pivot_in_window(df: pd.DataFrame, scan_days: int,
    ...
    start, end = resolve_scan_range(params or {})
    if start is not None and end is not None:
        # 특정기간: 시작일 ~ 현재봉 직전(및 종료일) 구간에서 탐색
        cur = _to_date(df.iloc[-1]["date"])
        cur_end = cur.date() if cur is not None and not pd.isna(cur) else end
        if end is not None:
            cur_end = min(cur_end, end)
        search_idxs = [
            idx for idx in df.index[:-1].tolist()
            if _date_in_range(df.loc[idx, "date"], start, cur_end)
        ]
    else:
        search_idxs = df.iloc[-(scan_days + 1):-1].index.tolist()
...
def _run_trade_simulation(df: pd.DataFrame, start_idx: int,
    ...
    result = {
        ...
        "stop_triggered": False, "stop_price_actual": 0, "stop_date": "",
        "profit1_triggered": False, "profit1_price": 0, "profit1_date": "",
        "profit2_triggered": False, "profit2_price": 0, "profit2_date": "",
