...
def _trade_record(code: str, name: str, t: dict) -> dict:
    return {
        ...
        "매수평균가"       : t.get("avg_price", 0),
        "손절일"           : t.get("stop_date", ""),
        "손절가"           : t.get("stop_price_actual", 0),
        "1차익절일"        : t.get("profit1_date", ""),
        "1차익절가"        : t.get("profit1_price", 0),
        "2차익절일"        : t.get("profit2_date", ""),
...
def detail_trades_df(df: pd.DataFrame) -> pd.DataFrame:
    """거래 상세 표시/다운로드용 컬럼만 추출"""
    if df.empty:
...
def summarize_by_stock(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for code, group in df.groupby("종목코드", sort=False):
        ...
        if buy_cnt > 0:
            pnl_col = "최종손익률" if "최종손익률" in buys.columns else "최종손익률(%)"
            pnl_series = buys[pnl_col]
...
def _empty_record(code: str, name: str, note: str) -> dict:
    return {
        "종목코드": code, "종목명": name,
        "저점캔들발생일": "", "기준캔들발생일": "",
        "1차매수가": 0, "2차매수가": 0, "3차매수가": 0, "매수평균가": 0,
        "손절일": "", "손절가": 0,
        "1차익절일": "", "1차익절가": 0,
        "2차익절일": "", "2차익절가": 0,
        "트레일링스탑발동": "-",
        "최종손익률": 0.0, "투자금액": 0, "손익금액": 0,
