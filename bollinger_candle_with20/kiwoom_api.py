...
class KiwoomAPI:
    ...
    def _parse_listing_df(self, df: pd.DataFrame) -> dict[str, str]:
        name_col = "회사명" if "회사명" in df.columns else df.columns[0]
        code_col = "종목코드" if "종목코드" in df.columns else df.columns[1]
        out: dict[str, str] = {}
        for _, row in df.iterrows():
            nm = self._norm_name(row[name_col])
            code = re.sub(r"\D", "", str(row[code_col]))
            if nm and code:
                out[nm] = code.zfill(6)
        return out

    def _fetch_krx_listing(self) -> dict[str, str]:
        """KRX 상장법인목록 다운로드 (여러 방식 시도)"""
        urls = [
        ...
        for url in urls:
            try:
                ...
                if df is None:
                    try:
                        df = pd.read_html(io.BytesIO(content), header=0)[0]
                    except Exception as e:
                        last_err = e
                        continue
                if df is None or df.empty:
                    continue
                mapping = self._parse_listing_df(df)
                if mapping:
                    logger.info("KRX 종목 목록 로드: %d개 (%s)", len(mapping), url[:40])
                    return mapping
            except Exception as e:
                last_err = e
                logger.warning("KRX 목록 시도 실패 [%s]: %s", url[:48], e)
        if last_err:
            logger.warning("KRX 종목 목록 로드 최종 실패: %s", last_err)
        return {}
        ...
        try:
            ...
            for item in items:
                ...
                if name in title or title in name:
                    exact = exact or code.zfill(6)
            return exact or first6
        except Exception as e:
            logger.warning("네이버 종목 검색 실패 [%s]: %s", name, e)
        return None

    def search_stock_code(self, stock_name: str) -> Optional[str]:
        """종목명/코드로 종목코드 검색. 실패 시 None."""
        name = self._norm_name(stock_name)
        if not name:
            return None
        ...
        try:
            ...
            if code:
                if self._name_code_map is None:
                    self._name_code_map = {}
                self._name_code_map[name] = code
                return code
            return None
        except Exception as e:
            logger.warning("종목코드 검색 실패 [%s]: %s", stock_name, e)
            try:
                return self._search_naver_code(name)
            except Exception:
                return None
