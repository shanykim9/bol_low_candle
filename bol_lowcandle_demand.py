...
from config import (
    ...
    calc_data_days_from_params, format_scan_period, scan_mode_options,
    scan_range_min_date, default_scan_start_ym, default_scan_end_ym,
    resolve_scan_range, SCAN_RANGE_YEARS_MAX,
...
def init_session():
    ...
    if not p.get("scan_start"):
        p["scan_start"] = default_scan_start_ym()
    if not p.get("scan_end"):
        p["scan_end"] = default_scan_end_ym()
...
def _sim_export_downloads(df_sim: pd.DataFrame, key: str):
    """시뮬레이션 CSV / Excel 다운로드 (종목별 요약 + 거래 상세)"""
    df_summary = summarize_by_stock(df_sim)
    df_detail  = detail_trades_df(df_sim)
    ...
    with pd.ExcelWriter(xlsx_buf, engine="openpyxl") as writer:
        df_summary.to_excel(writer, sheet_name="종목별요약", index=False)
        df_detail.to_excel(writer, sheet_name="거래상세", index=False)
    xlsx_buf.seek(0)
    ...
    with c1:
        st.download_button(
            ...
            file_name=f"simulation_{ts}.csv",
            mime="text/csv",
            key=f"{key}_csv",
        )
    with c2:
        st.download_button(
            label="📥 Excel 다운로드 (요약+상세)",
            data=xlsx_buf.getvalue(),
            file_name=f"simulation_{ts}.xlsx",
...
with st.sidebar:
    ...
    p["scan_mode"] = st.radio(
        "저점캔들 탐색 방식",
        scan_modes,
        index=scan_modes.index(p["scan_mode"]),
        horizontal=True,
    )
    ...
    if p["scan_mode"] == "상대기간":
        ...
        today = date.today()
        min_d = scan_range_min_date(today)
        year_opts = list(range(min_d.year, today.year + 1))

        if not p.get("scan_start"):
            p["scan_start"] = default_scan_start_ym(today)
        if not p.get("scan_end"):
            p["scan_end"] = default_scan_end_ym(today)

        try:
            s_y, s_m = map(int, str(p["scan_start"]).split("-")[:2])
        except ValueError:
            s_y, s_m = today.year, max(1, today.month - 6)
        try:
            e_y, e_m = map(int, str(p["scan_end"]).split("-")[:2])
        except ValueError:
            e_y, e_m = today.year, today.month

        s_y = min(max(s_y, year_opts[0]), year_opts[-1])
        e_y = min(max(e_y, year_opts[0]), year_opts[-1])

        st.caption(f"특정기간 설정 (최대 {SCAN_RANGE_YEARS_MAX}년 전 ~ 이번 달)")
        c_s1, c_s2, c_e1, c_e2 = st.columns(4)
        with c_s1:
            s_y = st.selectbox("시작 연도", year_opts, index=year_opts.index(s_y), key="scan_sy")
        start_months = list(range(min_d.month if s_y == min_d.year else 1, 13))
        if s_m not in start_months:
            s_m = start_months[0]
        with c_s2:
            s_m = st.selectbox(
                "시작 월", start_months,
                index=start_months.index(s_m), key="scan_sm",
                format_func=lambda m: f"{m}월",
            )
        with c_e1:
            e_y = st.selectbox("종료 연도", year_opts, index=year_opts.index(e_y), key="scan_ey")
        end_months = list(range(1, (today.month if e_y == today.year else 12) + 1))
        if e_m not in end_months:
            e_m = end_months[-1]
        with c_e2:
            e_m = st.selectbox(
                "종료 월", end_months,
                index=end_months.index(e_m), key="scan_em",
                format_func=lambda m: f"{m}월",
            )

        p["scan_start"] = f"{s_y:04d}-{s_m:02d}"
        p["scan_end"]   = f"{e_y:04d}-{e_m:02d}"
        p["data_days"]  = calc_data_days_from_params(p)

        start_d, end_d = resolve_scan_range(p)
        if start_d is None or end_d is None:
            st.error("⚠️ 시작 년월이 종료 년월보다 늦을 수 없습니다.")
        elif start_d < min_d:
            st.error(
                f"⚠️ 시작일은 {min_d.strftime('%Y-%m')} 이후여야 합니다. "
                f"(최대 {SCAN_RANGE_YEARS_MAX}년)"
            )
        else:
            st.caption(
                f"탐색 구간: {format_scan_period(p)} "
                f"({resolve_scan_days(p):,}거래일 추정) · "
                f"일봉 수집: {p['data_days']:,}일"
            )
    st.divider()
